...
def get_shebao_data(ss,company_name):
    a = []
    b = []
    try:
        for i in range(6, 12):
...
def handle_message(l):
    ...
    try:
        ...
        time.sleep(0.2)
        driver.find_element_by_xpath('//*[@id="cgsbTab"]/li[2]/a/span').click()
        time.sleep(0.2)
        list1 = [driver, cur_task_dir, "扣款结果查询", company_name]
        oss_url = screen(list1)
        save_image['扣款结果查询'] = {"扣款结果查询": oss_url}


        driver.get("https://etax.zhejiang.chinatax.gov.cn/zjgfdzswj/main/home/wdxx/nsrxx/index.html")
        driver.find_element_by_xpath('//*[@id="a2"]').click()
        time.sleep(0.2)
        list1 = [driver, cur_task_dir, "税种认定信息", company_name]
        oss_url = screen(list1)
        save_image['税种认定信息'] = {"税种认定信息": oss_url}
        ...
        for type1, num in [['社保', '31'], ['印花税', '11'], ['财务报表', '90'],
            ...
            if type1 == "社保":
                ...
                try:
                    ...
                    for suoyin, s in enumerate(li):
                        ti0 = ''.join(s.xpath('./td[3]/text()')).strip()
                        if ti0 == "是":
                            continue
                        detail_image = {}
                        for ii in range(5):
                        ...
                        ti1 = ''.join(s.xpath('./td[4]/text()'))
                        ti2 = ''.join(s.xpath('./td[3]/text()'))
                        ti3 = ''.join(s.xpath('./td[10]//text()'))
                        ...
                        if ti3 != "已申报已缴款" and ti3 != "已申报未缴款" and ti3 != "已申报缴款失败" and ti3!='已申报部分缴款':
                            continue
                        ...
                        if len(sss) == 0:
                            continue
                        if "补缴" not in ti0:
                            shebao_data = get_shebao_data(ss,company_name)
            ...
            if len(h_inter) != 0:
                status = 10  # IP被封
                save_image = {}
                logger.info("防火墙阻断{}".format(company_name))
                return
            ...
            for index, ele in enumerate(info):
                ...
                fuck = "".join(ele.xpath('./td[last()-1]//a/@href'))
                ...
                if type1 == "印花税":
                    time.sleep(2)
                    tree_detail = etree.HTML(driver.page_source)
                    detail_title = ''.join(tree_detail.xpath('//ul[@class="list-head-bg"][1]/li[1]//text()')).strip()
                    if detail_title == '':
                        driver.get(fuck_url)
                        time.sleep(3)
                        tree_detail = etree.HTML(driver.page_source)
                        detail_title = ''.join(
                            tree_detail.xpath('//ul[@class="list-head-bg"][1]/li[1]//text()')).strip()
                        if detail_title == '':
                            continue

                    detail_url = "https://etax.zhejiang.chinatax.gov.cn" + ''.join(
                        tree_detail.xpath('//ul[@class="list-head-bg"][1]/li[2]/a/@href'))
...
if __name__ == '__main__':
    ...
    main()
    # print(image_to_url("1.png"))
    # handle_message(
    #     [24592, "白熊芯（杭州）科技发展有限公司",
    #      "https://etax.zhejiang.chinatax.gov.cn/zjgfdzswj/main/home/wybs/index.html?ticket=ST-5467417-QahUZqAynioiCcbbxOgw-com.hz.zkxx.ydzhz"
    #         , "2022-08"]
    # )
    ...
    # callback_saas(21415, 0, {
    #     '常规申报列表': {'常规申报列表': 'http://oss-xwork.oss-cn-hangzhou.aliyuncs.com/71564208-0f56-4c0f-bd50-0198f63dbd69.jpg'},
    #     '社保': [], '印花税': [], '财务报表': [], '增值税': [], '企业所得税': []},
    #               1, {}
    #               )
