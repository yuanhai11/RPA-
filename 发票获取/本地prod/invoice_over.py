...
def main():
    ...
    with open("data.txt",encoding='utf-8')as pp:
        con = pp.readlines()
...
def get_data_by_proxy(headers,data,types,select_type,localDate,proxys):
    """
    存在问题，代理IP本身问题，导致数据获取不到，且20次获取不到后就下一个公司了。
    proxies = proxys[-1],
    """
    result1 = ""
    result = ""
    for rr in range(100):
        try:
            result1 = requests.get(
                'http://etax.zhejiang.chinatax.gov.cn/zjgfdacx/fpxxcx/query.do?rqq={}&rqz={}&fplx={}&cxfs={}&kpfxx=&fpdm=&fphm='.format(
                    localDate,localDate, types, select_type),
                # , data=data,
                headers=headers, proxies=proxys[-1],timeout=10
            ).text
            result = json.loads(result1)['resultObj']
            break
        except Exception as e:
            logger.info("get_data error"+str(e))
            if rr >= 99:
                result = None
                break
            if 'Tunnel connection failed: 407 Proxy Authentication Required' in str(e) or "白名单" in result1:
                lock.acquire()
                add_white()
                lock.release()
