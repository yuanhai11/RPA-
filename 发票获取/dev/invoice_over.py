...
def spider(cookie, import_data,proxys):
    ...
    for types,names in [
        ...
        for se in ['SPF','KPF']:
            ...
            time.sleep(0.5)
            result = get_data_by_proxy(headers,data,types,select_type,start_time,end_time,proxys)
...
def get_data_by_proxy(headers,data,types,select_type,start_time,end_time,proxys):

    for rr in range(20):
        try:
            result = requests.get(
                'https://etax.zhejiang.chinatax.gov.cn/zjgfdacx/fpxxcx/query.do?rqq={}&rqz={}&fplx={}&cxfs={}&kpfxx=&fpdm=&fphm='.format(
                    start_time,end_time, types, select_type)
                , data=data,
...
if __name__ == '__main__':
    ...
    # lock = Lock()
    # main()
    # data = {'handleTaxName': '李耀', 'handleTaxPwd': 'zlcw1401', 'taxAccount': '91330105MA2GNX4C4M', 'taxPwd': 'zlcw1401'}
    # get_login_cookie(data)
    pass
